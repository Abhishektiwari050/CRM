(() => {
    ...
    const checkAuth = async (requiredRole = null) => {
        ...
        if (requiredRole && user.role !== requiredRole && user.role !== 'admin') { location.href = user.role === 'manager' ? '/' : '/employee_dashboard_page/index.html'; return null }
        return user
    };
    const apiCall = async (endpoint, options = {}) => {
        ...
        if (res.status === 401) { clearToken(); location.href = '/'; throw new Error('Unauthorized') }
